...
def render(s):
    ...
    for w in s['windows']:
        ...
        <section class="card limit-card">
          ...
          <div class="big" style="color:{color}">%{w['remaining']} left</div>
          <div class="muted">Used: %{w['used']}</div>
          <div class="bar"><span style="width:{w['remaining']}%;background:{color}"></span></div>
          <div class="reset">Reset: {html.escape(w['reset'])}</div>
        </section>'''
    ...
    *{{box-sizing:border-box}} html{{min-height:100%;background:#050b12!important;color-scheme:dark!important}} body{{min-height:100svh;margin:0!important;background:#050b12!important;background-image:radial-gradient(circle at top,#102235 0,#050b12 52%,#03070c 100%)!important;color:#e8eef7!important;font-family:Inter,system-ui,-apple-system,Segoe UI,sans-serif;padding:clamp(8px,2vw,18px)!important}}
    ...
    </style></head><body><main class="wrap"><header><h1>Codex usage status</h1><div class="sub">Data refreshes every 30 seconds · Page loads instantly · Last update: {html.escape(s['updated'])} · Served at: {html.escape(s.get('servedAt','-'))}</div></header><div class="grid">{cards}</div>
    <section class="card meta"><div><b>Model:</b> <code>{html.escape(str(s['model']))}</code></div><div><b>Plan:</b> {html.escape(str(s['plan']))}</div><div><b>Context:</b> {html.escape(str(s['context']))}</div><div><b>Last turn tokens:</b> in {s['inputTokens']:,} / out {s['outputTokens']:,} / cache {s['cacheRead']:,}</div></section>
    </main></body></html>'''
...
class H(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        try:
            ...
            body = f'Error: {html.escape(str(e))}'.encode()
